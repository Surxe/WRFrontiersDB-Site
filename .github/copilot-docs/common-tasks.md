...
2. **Create type interface** in `src/types/{parseObject}.ts` (e.g., `export interface Weapon { ... }`)
3. **Add map from parseObject to url** in `src/components/PageRef.astro`'s `getPagePath` function
4. **Configure PageRef overload** in `src/utils/page_ref.ts`'s `getPageRefData` function
5. **Create list page**: `src/pages/{parseObject}s.astro` (properly pluralized) using `ParseObjectList`
6. **Create detail page**: `src/pages/{parseObject}s/[id]/[version].astro` with `getStaticPaths()`
7. **Call** `generateObjectStaticPaths("Objects/{parseObject}.json", true)`
