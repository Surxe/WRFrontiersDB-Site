...
class InterfaceDataValidator {
  ...
  async checkMissingAttribute(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        currentObjectId &&
        objects.some((obj: any) => obj.id === currentObjectId)
      ) {
  ...
  async generateInterfaceReport(objectType: string): Promise<void> {
    ...
    try {
      ...
      for (const field of interfaceFields) {
        ...
        if (field.optional) {
          console.log(
            `🔹 **${field.name}** (optional): ${result.present}/${result.total} objects (${result.percentage.toFixed(1)}%)`
          );
          if (result.missing > 0) {
            console.log(
              `   Missing from: ${result.missingObjects.slice(0, 5).join(', ')}${result.missingObjects.length > 5 ? '...' : ''}`
            );
          }
        } else {
          if (result.missing === 0) {
            console.log(
              `✅ **${field.name}** (required): Present in all objects`
            );
          } else {
            console.log(
              `❌ **${field.name}** (required): Missing from ${result.missing} objects!`
            );
            console.log(
              `   Missing from: ${result.missingObjects.slice(0, 5).join(', ')}${result.missingObjects.length > 5 ? '...' : ''}`
            );
      ...
      const extraFields = dataFields.filter(
        (field) => !interfaceFieldNames.has(field)
      );
   ...
   */
  private extractInterfaceFields(
    content: string
   ...
   */
  async printMissingAttributeReport(
    objectType: string,
    attributePath: string
  ): Promise<void> {
    try {
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log(
      '  node validate.js <objectType> report           # Generate full report'
    );
    console.log('');
