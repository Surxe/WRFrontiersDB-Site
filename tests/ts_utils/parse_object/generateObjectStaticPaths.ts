...
import { getParseObjects, getAllVersions } from '../../../src/utils/parse_object';
...
describe('generateObjectStaticPaths', () => {
  describe('basic functionality', () => {
    ...
    it('should generate paths for different object types', async () => {
      ...
      const modulePaths = await generateObjectStaticPaths('Objects/Module.json');
  ...
  describe('objectVersions tracking', () => {
    ...
    it('should have consistent objectVersions for same object ID', async () => {
      ...
      for (const [objectId, paths] of pathsByObjectId.entries()) {
        if (paths.length > 1) {
          const firstVersionsList = paths[0].props.objectVersions.sort().join(',');
          for (const path of paths) {
            expect(path.props.objectVersions.sort().join(',')).toBe(firstVersionsList);
          }
        }
      }
    });
    ...
    it('should list all versions where object exists', async () => {
      ...
      const expectedVersions = pathsForObject.map(p => p.params.version).sort();
      const actualVersions = result.find(p => p.params.id === objectId)!.props.objectVersions.sort();
  ...
  describe('prodReadyOnly filtering', () => {
    it('should include all objects when prodReadyOnly is false', async () => {
      const allPaths = await generateObjectStaticPaths('Objects/Module.json', false);
    ...
    it('should filter non-Ready objects when prodReadyOnly is true', async () => {
      const allPaths = await generateObjectStaticPaths('Objects/Module.json', false);
      const readyPaths = await generateObjectStaticPaths('Objects/Module.json', true);
    ...
    it('should only include Ready objects when prodReadyOnly is true', async () => {
      ...
      for (let i = 0; i < Math.min(5, result.length); i++) {
        const path = result[i];
        const modules = getParseObjects('Objects/Module.json', path.params.version);
    ...
    it('should exclude objects without production_status when prodReadyOnly is true', async () => {
      ...
      const nonReadyModule = Object.entries(allModules).find(([_, mod]) => 
  ...
  describe('edge cases', () => {
    ...
    it('should handle object types with fewer instances', async () => {
      const result = await generateObjectStaticPaths('Objects/PilotTalentType.json');
