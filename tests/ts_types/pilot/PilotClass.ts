...
describe('PilotClass interface vs data structure', () => {
  const dataPath = join(process.cwd(), 'WRFrontiersDB-Data', 'archive', '2025-12-09', 'Objects', 'PilotClass.json');
  ...
  it('should ensure every object has all required fields', () => {
    ...
    realObjects.forEach((obj, index) => {
      ...
      if (obj.badge) {
        const badge = obj.badge as Record<string, unknown>;
        expect(badge.image_path, `Object at index ${index} missing badge.image_path`).toBeDefined();
        expect(badge.hex, `Object at index ${index} missing badge.hex`).toBeDefined();
      }
    });
  });
  ...
  it('should flag if there are keys in data that are not in the interface', () => {
    ...
    expect(
      ...
      `Found unexpected keys in data that are not defined in PilotClass interface. Consider adding these to the interface or using the ParseObject indexer.`
    ).toEqual([]);
  });
});
