...
describe('PilotPersonality interface vs data structure', () => {
  const dataPath = join(process.cwd(), 'WRFrontiersDB-Data', 'archive', '2025-12-09', 'Objects', 'PilotPersonality.json');
  ...
  it('should flag if there are keys in data that are not in the interface', () => {
    ...
    expect(
      ...
      `Found unexpected keys in data that are not defined in PilotPersonality interface. Consider adding these to the interface or using the ParseObject indexer.`
    ).toEqual([]);
  });
});
