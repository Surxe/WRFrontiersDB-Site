...
export function isTitanShoulder(moduleId: string): boolean {
  return (
    moduleId.includes('ShoulderL') || 
    moduleId.includes('ShoulderR')
  ) && (
...
function getShoulderSideFromPreset(
  ...
  for (const preset of Object.values(presets)) {
    const moduleEntry = preset.modules.find(m => refToId(m.module_ref) === moduleId);
