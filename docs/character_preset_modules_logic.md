...
- **Result**: `Locust x3`
- *Why?* To keep the UI concise, we increment the total count rather than listing them separately per slot.
...
- If only **one** type of weapon exists in the preset: **HIDE group info**.
  - *Result*: `Locust x3` (Clean)
- If **multiple** types of weapons exist: **SHOW group info**.
  - *Result*: `Locust (Shoulder) x2`, `Buckshot (Torso) x1` (Helpful context)
